set(EMSCRIPTEN_ROOT_PATH
    "C:/Users/cakit/scoop/apps/emscripten/current/emscripten"
)
...
if(NOT DEFINED EMSCRIPTEN_ROOT_PATH)
    ...
    if(EXISTS "${GUESS_EMSCRIPTEN_ROOT_PATH}/emranlib")
        ...
        if("${EMSCRIPTEN_ROOT_PATH}" STREQUAL "")
            message(
                FATAL_ERROR
                "Could not locate the Emscripten compiler toolchain directory! Either set the EMSCRIPTEN environment variable, or pass -DEMSCRIPTEN_ROOT_PATH=xxx to CMake to explicitly specify the location of the compiler!"
            )
        endif()
    endif()
endif()
...
if(NOT EMSCRIPTEN_VERSION)
    ...
    if(NOT _cmake_compiler_result EQUAL 0)
        message(
            FATAL_ERROR
            "Failed to fetch Emscripten version information with command \"'${CMAKE_C_COMPILER}' -v\"!\n"
    ...
    if(NOT CMAKE_MATCH_1)
        message(
            FATAL_ERROR
            "Failed to regex parse Emscripten compiler version from version string: ${_cmake_compiler_output}"
        )
    endif()
...
execute_process(
    COMMAND "${EMSCRIPTEN_ROOT_PATH}/em-config${EMCC_SUFFIX}" "CACHE"
...
if(NOT _emcache_result EQUAL 0)
    message(
        FATAL_ERROR
        "Failed to find emscripten cache directory with command \"'${EMSCRIPTEN_ROOT_PATH}/em-config${EMCC_SUFFIX}' CACHE\"! Process returned with error code ${_emcache_result}."
    )
endif()
...
if(EMSCRIPTEN_FORCE_COMPILERS)
    ...
    if(NOT CMAKE_C_COMPILER_VERSION)
        ...
        if(NOT _cmake_compiler_result EQUAL 0)
            message(
                FATAL_ERROR
                "Failed to fetch compiler version information with command \"'${CMAKE_C_COMPILER}' -v\"! Process returned with error code ${_cmake_compiler_result}."
            )
        endif()
        if(NOT "${_cmake_compiler_output}" MATCHES "[Ee]mscripten")
            message(
                FATAL_ERROR
                "System LLVM compiler cannot be used to build with Emscripten! Check Emscripten's LLVM toolchain location in .emscripten configuration file, and make sure to point CMAKE_C_COMPILER to where emcc is located. (was pointing to \"${CMAKE_C_COMPILER}\")"
            )
        endif()
        string(
            REGEX MATCH
        ...
        if(NOT CMAKE_MATCH_1)
            message(
                FATAL_ERROR
                "Failed to regex parse Clang compiler version from version string: ${_cmake_compiler_output}"
            )
        endif()
        ...
        if(${CMAKE_C_COMPILER_VERSION} VERSION_LESS 3.9.0)
            message(
                WARNING
                "CMAKE_C_COMPILER version looks too old. Was ${CMAKE_C_COMPILER_VERSION}, should be at least 3.9.0."
            )
        endif()
    endif()
    ...
    if(NOT DEFINED CMAKE_CXX_IMPLICIT_INCLUDE_DIRECTORIES)
        set(CMAKE_CXX_IMPLICIT_INCLUDE_DIRECTORIES
            "${EMSCRIPTEN_SYSROOT}/include;${EMSCRIPTEN_SYSROOT}/include/c++/v1"
        )
    endif()
...
set(_em_pkgconfig_libdir
    "${EMSCRIPTEN_SYSROOT}/local/lib/pkgconfig"
    "${EMSCRIPTEN_SYSROOT}/lib/pkgconfig"
)
if("${CMAKE_VERSION}" VERSION_LESS "3.20")
    ...
    cmake_path(
        CONVERT
        "${_em_pkgconfig_libdir}"
        TO_NATIVE_PATH_LIST _em_pkgconfig_libdir
    )
endif()
...
option(
    ...
    "If set, static library targets generate LLVM bitcode files (.bc). If disabled (default), UNIX ar archives (.a) are generated."
    OFF
)
if(EMSCRIPTEN_GENERATE_BITCODE_STATIC_LIBRARIES)
    message(
        FATAL_ERROR
        "EMSCRIPTEN_GENERATE_BITCODE_STATIC_LIBRARIES is not compatible with the llvm backend"
    )
endif()
    ...
    foreach(jsFileList ${ARGN})
        foreach(jsfile ${jsFileList})
            ...
            set_target_properties(
                ${target}
                PROPERTIES LINK_DEPENDS "${linkdeps};${jsfile_abs}"
            )
